...
<body>
    ...
    <style>
        ...
        @media (max-width: 576px) {
            ...
            /* 2nd and 3rd columns (Quick Links and Customer Service) - side by side */
        ...
        <div class="base-container footer-container">
            ...
            <div class="footer-column">
                ...
                <div class="footer-links">
                    @php
                        $quickLinksMenu = \App\Models\Menu::where('name', 'Quick Links')
                            ->where('status', true)
                            ->first();
            ...
            <div class="footer-column">
                ...
                <div class="footer-links">
                    @php
                        $customerServiceMenu = \App\Models\Menu::where('name', 'Customer Service')->first();
                        $customerServiceLinks = $customerServiceMenu
                            ? $customerServiceMenu->menuItems()->where('status', true)->orderBy('order')->get()
                            : collect();
                    @endphp

                    @foreach ($customerServiceLinks as $item)
    ...
    <style>
        ...
        .cart-sidebar-remove svg path {
            transition: stroke 0.2s ease, fill 0.2s ease;
        }

        .cart-sidebar-remove:hover svg path {
            stroke: #b91c1c;
            fill: none;
        }
        ...
        .cart-drawer-actions .drawer-btn.primary {
            background: var(--primary-color);
            color: #fff;
            box-shadow: 0 10px 18px rgba(237, 26, 37, 0.35);
            box-shadow: 0 10px 18px color-mix(in srgb, var(--primary-color) 35%, transparent);
        }
