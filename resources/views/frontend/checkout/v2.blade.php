    ...
    <div class="checkout-order-info">
                    ...
                    <td class="cct-title">
                        <div style="display: flex; align-items: flex-start; gap: 15px;">
                            <div class="product-image" style="width: 60px; height: 60px; flex-shrink: 0; border-radius: 6px; overflow: hidden; border: 1px solid #e5e7eb;">
                                <img src="{{ asset('storage/' . $product->thumb_image) }}" alt="{{ $product->title }}" style="width: 100%; height: 100%; object-fit: cover;">
                            </div>
                            <div class="product-info">
                                <a href="" style="font-weight: 600; color: var(--secondary-color); display: block; margin-bottom: 5px;">
                                    {{ $product->title }}
                                </a>
                                <div class="product-variations">
                                    ...
                                    <div class="variation-item">
                                        ...
                                        <div class="quantity-controls" data-product-id="{{ $productId }}" style="display: inline-flex; align-items: center; margin-left: 10px; border: 1px solid #e5e7eb; border-radius: 4px; overflow: hidden;">
                                            <button type="button" onclick="updateCartQuantity('{{ $items->first()->id }}', 'decrease', this)" style="background: #f3f4f6; border: none; padding: 2px 8px; cursor: pointer; color: #6b7280; font-size: 14px; line-height: 1;">−</button>
                                            <input type="text" class="quantity-input" value="{{ $combinationDetails['qty'] }}" readonly style="width: 30px; padding: 2px 0; font-size: 12px; font-weight: 500; border: none; border-left: 1px solid #e5e7eb; border-right: 1px solid #e5e7eb; text-align: center; outline: none; background: #fff;">
                                            <button type="button" onclick="updateCartQuantity('{{ $items->first()->id }}', 'increase', this)" style="background: #f3f4f6; border: none; padding: 2px 8px; cursor: pointer; color: #6b7280; font-size: 14px; line-height: 1;">+</button>
                                        </div>
                                        <a href="{{ route('cart.destroy', $items->first()->id) }}" class="remove-btn" style="color: #ef4444; margin-left: 8px; font-size: 18px; text-decoration: none; display: flex; align-items: center;" title="Remove item">
                                            &times;
                                        </a>
                                    ...
                                    <div class="simple-item" style="font-size: 14px; color: #666; margin-top: 5px;">
                                        <div class="quantity-controls" data-product-id="{{ $productId }}" style="display: inline-flex; align-items: center; border: 1px solid #e5e7eb; border-radius: 4px; overflow: hidden; margin-right: 10px;">
                                            <button type="button" onclick="updateCartQuantity('{{ $items->first()->id }}', 'decrease', this)" style="background: #f3f4f6; border: none; padding: 2px 8px; cursor: pointer; color: #6b7280; font-size: 14px; line-height: 1;">−</button>
                                            <input type="text" class="quantity-input" value="{{ $item->qunt }}" readonly style="width: 30px; padding: 2px 0; font-size: 12px; font-weight: 500; border: none; border-left: 1px solid #e5e7eb; border-right: 1px solid #e5e7eb; text-align: center; outline: none; background: #fff;">
                                            <button type="button" onclick="updateCartQuantity('{{ $items->first()->id }}', 'increase', this)" style="background: #f3f4f6; border: none; padding: 2px 8px; cursor: pointer; color: #6b7280; font-size: 14px; line-height: 1;">+</button>
                                        </div>
                                        <a href="{{ route('cart.destroy', $items->first()->id) }}" class="remove-btn" style="color: #ef4444; margin-left: 8px; font-size: 18px; text-decoration: none; display: flex; align-items: center;" title="Remove item">
                                            &times;
        ...
        <div class="payment-method-section">
            ...
            <div>
                ...
                <h6 style="margin-top: 10px; font-weight: 600;">Online Payment</h6>
                ...
                <div class="payment-option">
                    ...
                    <label for="{{ $gw->provider }}" class="text-sm">
                        <i class="fas fa-credit-card me-1"></i> {{ $gw->name }}
                    </label>
                </div>
                @endforeach
                @endif
...
<script>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Update item total price in the row
                    const row = button.closest('tr');
                    if (row.length) {
                        row.find('.cct-price').text(response.new_total + '৳');
                    }
    ...
    // Make the whole shipping row select the option
