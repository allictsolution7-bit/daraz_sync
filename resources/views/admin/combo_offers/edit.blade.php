...
@section('title', 'Edit Combo Offer')
    ...
    <div class="container categories-container">
        ...
        <nav aria-label="breadcrumb" class="custom-breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Edit Combo Offer</li>
            </ol>
        </nav>
        ...
        <div class="glass-card">
            <!-- Header Section -->
            <div class="mb-4">
                <h1 class="page-header-title">Edit Combo Offer</h1>
                <p class="page-header-subtitle">Update store bundle promotions, pricing configurations, and active status.</p>
            </div>

            <!-- Display Validation / Session Alerts -->
            @if ($errors->any())
            ...
            <form action="{{ route('admin.combo_offers.update', $comboOffer) }}" method="POST" id="combo-offer-form">
                @csrf
                @method('PUT')
                <div class="row">
                        ...
                        <div class="form-section-card">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="product_id" id="product_id" class="form-select @error('product_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}" {{ old('product_id', $comboOffer->product_id) == $product->id ? 'selected' : '' }}>
                                                {{ $product->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" 
                                        value="{{ old('title', $comboOffer->title) }}" placeholder="e.g. Premium Solid Cotton T-Shirt Combo 3pcs" required>
                                    @error('title')
                                        <div class="text-error-small">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" id="description" rows="3" class="form-control @error('description') is-invalid @enderror" 
                                    placeholder="Write details or descriptions about this combo package...">{{ old('description', $comboOffer->description) }}</textarea>
                                @error('description')
                                    <div class="text-error-small">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" name="items_count" id="items_count" class="form-control @error('items_count') is-invalid @enderror" 
                                        value="{{ old('items_count', $comboOffer->items_count) }}" min="1" max="10" required>
                                    @error('items_count')
                                        <div class="text-error-small">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" name="combo_price" id="combo_price" class="form-control @error('combo_price') is-invalid @enderror" 
                                        value="{{ old('combo_price', $comboOffer->combo_price) }}" step="0.01" min="0" placeholder="e.g. 750.00" required>
                                    @error('combo_price')
                                        <div class="text-error-small">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" name="original_price" id="original_price" class="form-control @error('original_price') is-invalid @enderror" 
                                        value="{{ old('original_price', $comboOffer->original_price) }}" step="0.01" min="0" placeholder="e.g. 1000.00" required>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" name="sort_order" id="sort_order" class="form-control @error('sort_order') is-invalid @enderror" 
                                        value="{{ old('sort_order', $comboOffer->sort_order) }}" min="0">
                                    @error('sort_order')
                                        <div class="text-error-small">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="switch-wrapper form-check form-switch pt-2">
                                        ...
                                        <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" {{ old('is_active', $comboOffer->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="col-lg-5">
                        <div class="form-section-card">
                            ...
                            <div id="combo-items">
                                @forelse($comboOffer->items as $index => $item)
                                    <div class="combo-item" data-index="{{ $index }}">
                                        <div class="row">
                                            <div class="col-12 mb-2">
                                                <label class="form-label">Product <span class="text-danger">*</span></label>
                                                <select name="items[{{ $index }}][product_id]" class="form-select product-select @error('items.'.$index.'.product_id') is-invalid @enderror" required>
                                                    <option value="">Select Product</option>
                                                    @foreach($products as $product)
                                            ...
                                            <div class="col-12 mb-2">
                                                ...
                                                <select name="items[{{ $index }}][variation_combination_id]" class="form-select variation-select @error('items.'.$index.'.variation_combination_id') is-invalid @enderror">
                                                    <option value="">Select Variation</option>
                                                    @php
                                                        $variations = \App\Models\VariationCombination::where('product_id', $item->product_id)
                                                            ->where('is_active', true)
                                                            ->get();
                                                    @endphp
                                                    @foreach($variations as $variation)
                                                        <option value="{{ $variation->id }}" {{ old('items.'.$index.'.variation_combination_id', $item->variation_combination_id) == $variation->id ? 'selected' : '' }}>
                                                            {{ $variation->display_name || $variation->name || 'Variation ' . $variation->id }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                @error('items.'.$index.'.variation_combination_id')
                                                    <div class="text-error-small">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="col-7">
                                                ...
                                                <input type="number" name="items[{{ $index }}][sort_order]" class="form-control @error('items.'.$index.'.sort_order') is-invalid @enderror" 
                                                    value="{{ old('items.'.$index.'.sort_order', $item->sort_order) }}" min="0">
                                            ...
                                            <div class="col-5 pt-4 text-end">
                                                <div class="form-check form-switch d-inline-block text-start">
                                                    ...
                                                    <input type="checkbox" class="form-check-input" id="item_active_{{ $index }}" name="items[{{ $index }}][is_active]" value="1" {{ old('items.'.$index.'.is_active', $item->is_active) ? 'checked' : '' }}>
                                                    <label class="form-check-label text-muted" for="item_active_{{ $index }}" style="font-size: 0.8rem;">Active</label>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                @empty
                                    <!-- Fallback empty item if somehow items list is empty -->
                                    <div class="combo-item" data-index="0">
                                        <div class="row">
                                            ...
                                            <div class="col-12 mb-2">
                                                <label class="form-label">Variation (Optional)</label>
                                                <select name="items[0][variation_combination_id]" class="form-select variation-select">
                                                    <option value="">Select Variation</option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-7">
                                                <label class="form-label">Sort Order</label>
                                                <input type="number" name="items[0][sort_order]" class="form-control" value="0" min="0">
                                            </div>

                                            <div class="col-5 pt-4 text-end">
                                                <div class="form-check form-switch d-inline-block text-start">
                                                    <input type="hidden" name="items[0][is_active]" value="0">
                                                    <input type="checkbox" class="form-check-input" id="item_active_0" name="items[0][is_active]" value="1" checked>
                            ...
                            <div class="d-flex gap-2 mt-3">
                                ...
                                <button type="button" id="remove-item" class="btn-action-danger-outline" style="{{ count($comboOffer->items) > 1 ? 'display: inline-flex;' : 'display: none;' }}">
                        ...
                        <div class="d-flex align-items-center justify-content-between gap-3 mt-4">
                            ...
                            <button type="submit" class="btn-submit-premium" id="submit-btn">
                                <i class="fa-solid fa-floppy-disk"></i> Update Combo
                            </button>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let itemIndex = {{ count($comboOffer->items) > 0 ? count($comboOffer->items) : 1 }};
    ...
    function addNewItem() {
        ...
        checkboxes.forEach(function(checkbox) {
            ...
            // Update ID and label
            if (checkbox.id) {
                const newId = checkbox.id.replace(/_\d+$/, '_' + itemIndex);
                checkbox.id = newId;
                const label = newItem.querySelector('label[for^="item_active_"]');
                if (label) {
