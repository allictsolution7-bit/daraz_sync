...
@section('styles')
    <style>
        ...
        /* Elegant Publisher Logo Image Frame */
    ...
    <div class="container categories-container">
        ...
        <div class="glass-card">
            ...
            <div class="row align-items-center mb-4">
                <div class="col-md-6">
                    <h1 class="page-header-title">Publishers Directory</h1>
                    <p class="page-header-subtitle">Manage publishing houses, official contact emails, locations, and branding logos.</p>
                </div>
                <div class="col-md-6 text-md-end">
                    <div class="action-toolbar">
                        <a href="{{ route('admin.publishers.create') }}" class="btn-add-category">
                            <i class="fa-solid fa-plus"></i> Add New Publisher
                        </a>
                    </div>
                </div>
            </div>
                ...
                <table class="table premium-table">
                    ...
                    <tbody>
                        @forelse($publishers as $publisher)
                            <tr>
                                <td class="font-monospace text-muted">#{{ $publisher->id }}</td>
                                <td>
                                    @if ($publisher->logo)
                                        <div class="category-image-frame">
                                            <img src="{{ asset($publisher->logo) }}" alt="{{ $publisher->name }}">
                                        </div>
                                    @else
                                ...
                                <td class="fw-semibold text-slate-800">{{ $publisher->name }}</td>
                                <td class="text-muted">{{ $publisher->email }}</td>
                                <td class="text-secondary text-wrap-column">{{ $publisher->address }}</td>
                                <td class="text-end">
                                    <div class="d-inline-flex gap-1" role="group">
                                        <a href="{{ route('admin.publishers.edit', $publisher) }}" 
                                           class="btn-action-custom btn-action-edit" title="Edit Publisher Details">
                                        ...
                                        <form action="{{ route('admin.publishers.destroy', $publisher) }}" method="POST" class="d-inline">
                                            ...
                                            <button type="submit" class="btn-action-custom btn-action-delete" 
                                                    title="Delete Publisher" onclick="return confirm('Are you sure you want to delete this publisher?');">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    ...
                                    No publishers found in the directory.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
