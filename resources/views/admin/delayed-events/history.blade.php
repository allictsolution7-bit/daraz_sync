...
@section('title', 'Dispatcher Log History')
...
<style>
    ...
    .client-avatar-fired { background: #ecfdf5; border: 1px solid #a7f3d0; color: #059669; }
    .client-avatar-failed { background: #fef2f2; border: 1px solid #fecaca; color: #dc2626; }
    .client-avatar-pending { background: #fffbeb; border: 1px solid #fde68a; color: #d97706; }
    ...
    .status-badge-fired { background: #ecfdf5; border: 1px solid #a7f3d0; color: #059669; }
    .status-badge-failed { background: #fef2f2; border: 1px solid #fecaca; color: #dc2626; }
    .status-badge-pending { background: #fffbeb; border: 1px solid #fde68a; color: #d97706; }

    /* Diagnostic log text overlay */
    .error-log-notice {
        width: 100%;
        background: #fef2f2;
...
@section('content')
<div id="history-events-page" class="container-fluid px-4 py-4">
    <!-- Premium Header -->
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div>
                <h1>
                    ...
                    Conversions Log Archive
                </h1>
                <p class="text-muted mb-0">Record history of all fired or failed dispatcher event relays.</p>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('admin.delayed-events.settings') }}" class="btn-premium btn-premium-outline">
                    <i class="fas fa-cog"></i> Config Settings
                </a>
                <a href="{{ route('admin.delayed-events.pending') }}" class="btn-premium btn-premium-outline">
        ...
        <div class="pipeline-container">
            @foreach($events as $event)
                @php
                    ...
                    $statusClass = 'pipeline-card-pending';
                    $statusBadgeClass = 'status-badge-pending';
                    $statusIcon = 'fa-pause-circle';
                    $avatarClass = 'client-avatar-pending';
                    if ($event->fire_failed) {
                        $statusLabel = 'Failed';
                        $statusClass = 'pipeline-card-failed';
                        $statusBadgeClass = 'status-badge-failed';
                        $statusIcon = 'fa-exclamation-circle';
                        $avatarClass = 'client-avatar-failed';
                    } elseif ($event->fired_at) {
                        $statusLabel = 'Fired';
                        $statusClass = 'pipeline-card-fired';
                        $statusBadgeClass = 'status-badge-fired';
                        $statusIcon = 'fa-check-circle';
                        $avatarClass = 'client-avatar-fired';
                    }
                @endphp
                <div class="pipeline-card {{ $statusClass }}">
                    <!-- Client Details Block -->
                    <div class="client-details-block">
                        <div class="client-avatar-icon {{ $avatarClass }}">
                            <i class="fas fa-history"></i>
                        </div>
                    ...
                    <div class="event-meta-block">
                        ...
                        <div>
                            <span class="pipeline-status-badge {{ $statusBadgeClass }}">
                    ...
                    <div class="event-meta-block">
                        ...
                        <span class="event-id-meta">
                            @if($event->fired_at)
                                {{ $event->fired_at->diffForHumans() }}
                            @else
                                <span class="text-muted">Not Transmitted</span>
                            @endif
                        </span>
                    </div>
                    ...
                    <!-- Fail diagnostics box if active -->
                    @if($event->fire_failed && $event->fire_error)
                        <div class="error-log-notice">
                            <i class="fas fa-bug"></i>
                            <span><strong>Diagnostic Error:</strong> {{ $event->fire_error }}</span>
                        </div>
                    @endif
                </div>
            @endforeach
    ...
    @else
        <div class="premium-card text-center text-muted py-5">
            <i class="fas fa-history fa-4x mb-3 text-gray-300" style="color: #cbd5e1;"></i>
            <h4 class="font-weight-bold text-dark mb-1" style="font-size: 1.25rem;">Archive logs are Empty</h4>
            <p class="mb-0 text-muted">No historical conversion events are recorded in the dispatcher archives.</p>
        </div>
    @endif
</div>
@endsection
